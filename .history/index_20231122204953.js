...
app.delete("/book-author-delete/:isbn/:id", async (req,res) => {
    // console.log(req.params);
    const {isbn,id} = req.params; //user requested
    // id = Number(id); //data manipulate
    let deleteAuthorFromBook = await BookModel.findOne({ISBN: isbn});
    if (deleteAuthorFromBook===null) 
        { 
            return res.json( { "error": `No Book found for the ISBN of ${isbn}` } ); 
        }
    else {
        deleteAuthorFromBook.authors.remove(id);
        const updateBook = await BookModel.findOneAndUpdate({ISBN: isbn},  deleteAuthorFromBook , {new: true})
...
app.delete("/author-book-delete/:id/:isbn", async (req,res) => {
    ...
    // id = parseInt(id);
    const deleteBookInAuthorsDb = await AuthorModel.findOne({books: isbn});
    if (deleteBookInAuthorsDb===null) 
        {
            return res.json( { "error": `No Book found for the ISBN of ${isbn}` } );
        }
        else {
            deleteBookInAuthorsDb.books.remove(isbn);
            const updateAuthor = await AuthorModel.findOneAndUpdate({books: isbn}, deleteBookInAuthorsDb , {new: true});
            return res.json( {authorUpdated: updateAuthor, message: "Book was deleted from the authors Database !!!"} );
        }
});
...
app.delete("/publication-delete/:id", async (req,res) => {
    console.log(req.params);
    const {id} = req.params;
    const deletePublication = await PublicationModel.deleteOne({id}); 
