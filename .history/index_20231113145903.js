...
app.get("/author-id/:id", (req,res) => {
    ...
    console.log(id);
    const getSpecificAuthor = db.authors.filter((book) => book.id === id);
...
app.get("/author-isbn/:isbn", (req,res) => {
    ...
    const getSpecificIsbn = db.books.filter((book)=> book.ISBN === isbn);
    const authorIdList = getSpecificIsbn.map((book) => book.authorId === isbn);
    
    if(getSpecificIsbn.length===0) {
        return res.json({"error":`No Books Found with this Isbn of ${authorIdList}`}) ;
    }
    return res.json(authorIdList[0]);
    }
);
