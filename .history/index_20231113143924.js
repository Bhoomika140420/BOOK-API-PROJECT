...
app.get("/author-isbn/:isbn", (req,res) => {
    ...
    const getSpecificIsbn = db.books.filter((author)=> author.ISBN === isbn);
    const authorIdList = db.getSpecificIsbn.map((author) => author.authorId === isbn);
    
    if(getSpecificIsbn.length===0) {
        return res.json({"error":"No Books Found with this Isbn"}) ;
    }
